...
class DataIntegrity:
    ...
    async def issue_credential(self, credential, options):
        ...
        existing_proofs = existing_proofs if isinstance(existing_proofs, list) else [existing_proofs]
        if 'previousProof' in options:
            matching_proofs = []
            previous_proof = options['previousProof'] if isinstance(options['previousProof'], list) else [options['previousProof']]
            for proof_id in previous_proof:
                proof = next((proof for proof in existing_proofs if proof['id'] == proof_id), None)
                if not proof:
                    raise ""
    ...
    async def verify_credential(self, vc, options):
        ...
        verification_results = {
            'verified': all(all_verifications)
        } 
